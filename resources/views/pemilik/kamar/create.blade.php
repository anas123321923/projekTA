...
@section('scripts')
  ...
  <script type="text/javascript">
    ...
    // Auto-select Provinsi Papua Barat dan Kabupaten Manokwari
    $(document).ready(function() {
      var defaultProvinceId = 91; // ID Papua Barat
      var defaultRegencyId = 91;  // ID Manokwari

      // Pilih Provinsi Papua Barat
      $('#province').val(defaultProvinceId).trigger('change');

      // Setelah trigger provinsi change, tunggu hasil ajax kabupaten
      $.ajax({
        type:"GET",
        url:"{{url('select-regency')}}?province_id=" + defaultProvinceId,
        success:function(res){
          if(res){
            $("#regency").empty();
            $("#regency").append('<option value="">-- Pilih Kabupaten --</option>');
            $.each(res,function(key,value){
              var selected = (value.id == defaultRegencyId) ? 'selected' : '';
              $("#regency").append('<option value="'+value.id+'" '+selected+'>'+value.name+'</option>');
            });

            // Trigger change untuk ambil district
